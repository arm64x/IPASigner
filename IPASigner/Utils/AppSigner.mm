...
- (void)signAppWithAplication:(ALTApplication *)application certificate:(ALTCertificate *)certificate provisioningProfile:(ALTProvisioningProfile *)profile logHandler:(void (^)(NSString *log))logHandler completionHandler:(void (^)(BOOL success, NSError *error, NSURL *url))completionHandler {
    ...
    if (application == nil) {
        ...
        completionHandler(NO, [NSError errorWithDomain:AltSignErrorDomain code:ALTErrorInvalidApp userInfo:nil], resignedIPAURL);
        return ;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        char *argv[] = {"-k", p12, "-p", password, "-m", prov, "-i", ipa};
        for (int i = 0; i < 8; i += 1) {
            char* option = argv[i];
            printf("%s\n", option);
        }
        ...
        if (res == 0) {
            ...
            logHandler(@"开始打包IPA，请勿关闭ABox或退到后台");
